...
class Inventory {
	...
	public function loginAdmin($adminEmail, $adminPassword){
		...
		$sqlQuery = "
			SELECT adminId, adminEmail, adminPassword, adminName, adminType
    ...
    $sqlQuery = "SELECT p.pid, p.pname, p.model, p.quantity as product_quantity, s.quantity as received_quantity
    ...
    while ($inventory = mysqli_fetch_assoc($result)) {
        ...
        $inventoryInHand = ($inventory['product_quantity'] + $inventory['received_quantity']);
